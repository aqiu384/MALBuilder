...
url = 'http://test.animeadvice.me/api/v1/animestats/{"$and":[{"aired_from":{"$gte":1414994400,"$lt":1435035600}}]}/?filters={"$and":[{"aired_from":{"$gte":1414994400,"$lt":1435035600}}]}&fields=["aired_from"]&limit=0&query_limit=50&group_limit=100&sort_type=max&skip=50&sort=[["weight",-1]]&group_sort=[["aired_from",1]]'
url2 = 'http://test.animeadvice.me/api/v1/animelist/?filters={"$and":[{"aired_from":{"$gte":1414994400,"$lt":1435035600}}]}&fields=[]&limit=0&query_limit=50&group_limit=100&sort_type=max&skip=50&sort=[["weight",-1]]&group_sort=[["aired_from",1]]&format=json'

print(ureq.urlopen(ureq.Request(url2)).read())
data = json.loads(ureq.urlopen(url2).read().decode())
print(data)
